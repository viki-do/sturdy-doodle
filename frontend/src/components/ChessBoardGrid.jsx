...
const ChessBoardGrid = ({ gameLogic, onMouseDown, onMouseUp }) => {
    ...
    for (let i = 0; i < 8; i++) {
        for (let j = 0; j < 8; j++) {
            ...
            // 1. Kiszámoljuk a színt ELŐRE minden mezőre
            const hoverOutlineColor = sqName === selectedSquare 
                ? '#E7EDBD' 
                : (isDark ? '#CEDAC3' : '#F8F8EF');

            // 2. Csak egy egyszerű igaz/hamis kapcsolót használunk a stílusnak
            const isHoverActive = isDragging && selectedSquare && sqName === hoverSquare;
            // Csak a stílust kapcsoljuk
            let outlineStyle = (isDragging && selectedSquare && sqName === hoverSquare) ? 'solid' : 'none';
    ...
    <div 
        ...
        style={{ 
            outlineWidth: '3px',
            outlineStyle: isHoverActive ? 'solid' : 'none', // Csak ezt kapcsolgatjuk
            outlineColor: hoverOutlineColor,               // Ez fixen a te színed
            outlineOffset: '-3px',
